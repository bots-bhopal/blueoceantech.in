                ...
                <div class="card">
                    ...
                    <div class="card-body">
                        <form id="slink" class="form-horizontal" action="{{ route('admin.updateSlinks', $slink->id) }}" method="POST" onsubmit="store(event)">
                            ...
                            <div class="form-group row">
                                <label for="target" class="col-sm-2 control-label">{{ __('Status') }}<span class="text-danger">*</span></label>

                                <div class="col-sm-10">
                                    <select class="form-control" name="target">
                                       <option value="">{{ __('----- Select Target -----') }}</option>
                                       <option value="_blank" {{ $slink->target == '_blank' ? 'selected' : '' }}>{{ __('_blank') }}</option>
                                       <option value="_self" {{ $slink->target == '_self' ? 'selected' : '' }}>{{ __('_self') }}</option>
                                      </select>
